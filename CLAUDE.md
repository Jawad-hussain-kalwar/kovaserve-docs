...
8. **Never leak internal development artifacts into user-facing docs.** Users have no context for and no access to the blueprint, positioning brief, vertical slices, SLICE-STATUS, ADRs, user stories, eval scenarios, or any other internal planning surface. Referencing them by name, citing them, etc breaks the finished-product illusion.
   - **No references** (by name, link, or paraphrase) to: the blueprint, the positioning brief, vertical slices, slice status, ADRs, user stories, eval scenarios, internal Pass numbers, internal modules, `kova-v1`.
   - **No internal vocabulary**: `v1`, `v2`, `Pass N`, `slice`, `ADR-NNN`, "the blueprint says", "per ADR", "as positioned".
   - **`Layer 1 / Layer 2 / Layer 3` is product vocabulary, not internal-program vocabulary** — but only inside `concepts/layers/` where it teaches the user where KovaServe sits in their AI infrastructure stack (substrate / control plane / runtime). It is **not** for engineering walls-of-text elsewhere; if you find yourself writing "Layer 2 owns X" on a non-layers page, rewrite as "KovaServe owns X."
   - **Voice is "this is the truth", present tense, declarative.** Always "KovaServe does X." Never "KovaServe will do X", "X is not yet supported", "the product is not X", "this will ship in …", "the roadmap includes …".
9. **Banned phrases** anywhere in `content/docs/`: "not yet shipped", "deferred", "reserved for", "coming in slice", "v1 status", "v1 deviation", "in v1 only", "future capability", "when X ships", "accepted but not acted upon", "pinned to empty", "scaffolding", "the blueprint", "per ADR", "ADR-", "Pass 1/2/3/…", "vertical slice", "slice status", "user story", "eval scenario", "v1", "v2", "kova-v1". If you catch yourself reaching for any of these, rewrite in present tense as a finished feature with no internal references. (`Layer 1 / Layer 2 / Layer 3` is permitted on `concepts/layers/` pages only — see rule 8.)
